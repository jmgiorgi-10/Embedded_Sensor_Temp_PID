...
// Closes all LabJack device handles in the current process. Intended for users who want to release devices so a different
// process can claim them, without closing their current process. This function does not release all resources if stream
// mode was ran, and it is not recommended to close and reopen thousands of times in a process runtime.
...
3.50: Fixed 64-bit compatibility for eAIN.
3.51: Added void parameter to the Close function for C.
...
3.52: Reverted constants to const for backwards compatibility reasons.
3.53: Updated a U3/U6 stream thread delay to prevent stream buffer overflows if a stream is
      restarted too fast.
*/
